...
static IplImage *v_image_processed = NULL;	/**< Processed gray image */
...
static IplImage *c_image_processed = NULL;	/**< Processed gray image */
...
int open_video(const char* filename)
{
	...
	v_image_processed = cvCreateImage(cvGetSize(v_capimg), v_capimg->depth, 1);
...
IplImage* read_from_video(int frame_no )
{
	...
	// cvCvtColor( v_image, v_image_processed, CV_BGR2GRAY );
	// cvEqualizeHist(v_image_processed, v_image_processed);

	return v_image;
}

bool open_camera(int index, int camWidth, int camHeight)
{
	c_capture = cvCaptureFromCAM(index);
	if(!c_capture)	return false;
	cvSetCaptureProperty(c_capture, CV_CAP_PROP_FRAME_HEIGHT, camHeight);
	cvSetCaptureProperty(c_capture, CV_CAP_PROP_FRAME_WIDTH, camWidth);
	cvSetCaptureProperty(c_capture, CV_CAP_PROP_FPS, 15);
	...
	c_image_processed = cvCreateImage(cvGetSize(c_capimg), c_capimg->depth, 1);
...
{
	if(!cvGrabFrame(c_capture)){
		printf("Could not grab a frame\n");
		exit(0);
	}
	else {
		//printf("Grabbed a frame\n");
		c_capimg = cvRetrieveFrame(c_capture);
	}
	
	if(c_capimg->origin != IPL_ORIGIN_TL)
		cvFlip(c_capimg, c_image);
	else
		cvCopy(c_capimg, c_image);

	// cvCvtColor( c_image, c_image_processed, CV_BGR2GRAY );
	// cvEqualizeHist(c_image_processed, c_image_processed);
